...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        prog="af3bench",
        description="AF3 condition-comparison pipeline (ensemble-aware).",
    )
    ...
    p.add_argument("--version", action="version", version=f"af3bench {__version__}")
    return p
...
def main(argv: Optional[List[str]] = None) -> None:
    ...
    log.info("=== af3bench %s ===", __version__)
