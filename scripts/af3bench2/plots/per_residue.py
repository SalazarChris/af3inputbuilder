...
def plot_profile(
    ...
    # Check if this is a failed condition (§ 7b)
    is_failed = style.is_failed_condition(name)
    ...
    title = f"Per-residue displacement vs baseline: {style.short_labels([name])[0]}"
    if tier != "ok":
    ...
    if is_failed:
        banner_text = ("⚠ COLLAPSED PREDICTION — pTM ≈ 0.20, mean PAE ≈ 30 Å.\n"
    ...
    # Significance markers: remove for failed conditions (§ 7b)
    ...
    ax_pl.plot(x, ref_pl, color=ref_color, linewidth=0.9, label="baseline pLDDT")
    ...
    for pos in ptm_positions or [101, 102]:  # Default to common PTM sites if none specified
        if pos in res_set:
