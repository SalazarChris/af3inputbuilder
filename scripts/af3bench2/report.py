...
def _write_markdown(f: dict, path: Path) -> None:
    ...
    if summary:
        ...
        multimodal_conds = [r for r in summary if r.get("structural_shift") == "multimodal_ensemble"]
        if multimodal_conds:
    ...
    lines.append("| Condition | RMSD (mean [95% CI]) | TM-score | Residues moving (FDR<0.05) | Confidence |")
    lines.append("|---|---|---|---|---|")
    for row in f.get("distances", []):
        ...
        lines.append(
            f"| {row['condition']} | {rmsd} | {tm} | {row.get('n_significant', 0)} | {tier} |"
        )
    lines.append("")
...
{thumb}
<div class="meta">
 af3bench2 v{html.escape(_ver)} · baseline: {html.escape(str(f.get('baseline','')))} · generated {date.today().isoformat()}
