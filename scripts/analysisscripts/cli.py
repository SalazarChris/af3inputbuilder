"""
Command-line interface for analysisscripts.
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        prog="af3_analysis",
        description="AF3 condition-comparison pipeline (ensemble-aware, analysisscripts build).",
    )
    ...
    p.add_argument("--no-tm", action="store_true",
                   help="Disable TM-score computation (enabled by default if tmtools available).")
    p.add_argument("--version", action="version", version=f"af3_analysis {__version__}")
    return p
...
def main(argv: Optional[List[str]] = None) -> None:
    ...
    # TM-score enabled by default if tmtools available (Cycle 3 fix)
    compute_tm = HAS_TMTOOLS and not args.no_tm
    if args.no_tm:
        log.info("TM-score computation disabled by --no-tm flag.")
    elif not HAS_TMTOOLS:
        log.warning("tmtools not installed; TM-score computation skipped.")

    style.configure(dpi=args.dpi, formats=[f.strip() for f in args.formats.split(",") if f.strip()])

    log.info("=== af3_analysis %s ===", __version__)
