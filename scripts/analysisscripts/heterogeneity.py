...
def _assign_tier(n_clusters: int, dominant_fraction: float, is_collapsed: bool = False) -> str:
    """Within-condition heterogeneity tier, keyed on the *occupancy of the
    dominant structural cluster* rather than the raw cluster count.

    Raw n_clusters alone is dominance-blind: a 95%/5% split (one state plus a
    single outlier replicate) would otherwise be tiered the same as a genuine
    55%/45% bistable ensemble.  The dominant fraction is the quantity that
    distinguishes an effectively unimodal ensemble from a multimodal one, so it
    drives the tier:

      low       dominant_fraction >= 0.80  (effectively unimodal; minor
                clusters are sampling noise)
      moderate  0.50 <= dominant_fraction < 0.80  (a major state with a
                substantial secondary population)
      high      dominant_fraction < 0.50  (no majority state — genuinely
                multimodal / dispersed)

    The 0.80 / 0.50 occupancy boundaries are reporting conventions (labelled as
    such in outputs); the underlying dominant_fraction and cluster occupancies
    are always reported alongside so the tier never replaces the observable.
    """
    if is_collapsed:
        return "collapsed"
    if n_clusters <= 1:
